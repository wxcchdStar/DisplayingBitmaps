    ...
    private class BitmapWorkerTask extends AsyncTask<Void, Void, BitmapDrawable> {
        ...
        protected BitmapDrawable doInBackground(Void... params) {
            ...
            if (bitmap != null) {
                ...
                if (mImageCache != null) {
                    mImageCache.addBitmapToCache(dataString, drawable, mImageCacheParams);
                }
            }
        ...
        protected void onPostExecute(BitmapDrawable value) {
            ...
            if (isCancelled() || mExitTasksEarly) {
                value = null;
                // 不加这句会引起BUG：当第一次加载图片而mExitTasksEarly=true，
                // 在第二次加载同一个图片时，取先调用cancelPotentialWork方法，而cancelPotentialWork方法会返回false，
                // 也就是说图片不会在重新加载了。然而第一次加载是不成功的（因为mExitTasksEarly=true）
                mData = null;
            }
